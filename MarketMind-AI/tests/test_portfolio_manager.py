...
-----------------------------
Purpose:
    Unit tests for the Portfolio Management Part 3 concrete module:
    `strategies.portfolio_management.portfolio_manager.PortfolioManager`.

Mirrors the local-factory / assertion style already used by
`tests/test_portfolio_management.py` (Part 1) and
`tests/test_basic_portfolio_manager.py` (Part 2), both left untouched
by this change, and the fake-sub-component injection style of
`tests/test_strategy_aggregator.py`/`tests/test_signal_aggregator.py`
one and two layers up, respectively.
...
integration section at the bottom combines real `BasicPortfolioManager`
instances to prove the module actually reuses Portfolio Management
Part 2 end-to-end, as required.
...
class TestConstruction(unittest.TestCase):
    def test_defaults_to_single_basic_portfolio_manager(self):
        ...
        self.assertEqual(list(manager._managers), ["BasicPortfolioManager"])
        self.assertIsInstance(manager._managers["BasicPortfolioManager"], BasicPortfolioManager)
        self.assertEqual(manager.weights, {"BasicPortfolioManager": 1.0})
        self.assertEqual(manager.allow_threshold, 0.0)
        self.assertEqual(manager.name, "PortfolioManager")

    def test_is_a_base_portfolio_manager(self):
        self.assertIsInstance(PortfolioManager(), BasePortfolioManager)

    def test_custom_managers_and_name(self):
        mgr_a = _FakePortfolioManager(name="A")
        mgr_b = _FakePortfolioManager(name="B")
        manager = PortfolioManager(managers=[mgr_a, mgr_b], name="Custom")
        self.assertEqual(list(manager._managers), ["A", "B"])
    ...
    def test_rejects_empty_manager_list(self):
        with self.assertRaises(PortfolioManagerConfigurationError):
            PortfolioManager(managers=[])

    def test_rejects_non_manager_item(self):
        with self.assertRaises(PortfolioManagerConfigurationError):
            PortfolioManager(managers=["not a manager"])  # type: ignore[list-item]
    ...
    def test_custom_weights(self):
        mgr_a = _FakePortfolioManager(name="A")
        mgr_b = _FakePortfolioManager(name="B")
        manager = PortfolioManager(managers=[mgr_a, mgr_b], weights={"A": 2.0, "B": 0.5})
        self.assertEqual(manager.weights, {"A": 2.0, "B": 0.5})

    def test_weights_default_to_one_when_unspecified(self):
        mgr_a = _FakePortfolioManager(name="A")
        mgr_b = _FakePortfolioManager(name="B")
        manager = PortfolioManager(managers=[mgr_a, mgr_b], weights={"A": 3.0})
        self.assertEqual(manager.weights, {"A": 3.0, "B": 1.0})

    def test_rejects_unknown_weight_key(self):
        with self.assertRaises(PortfolioManagerConfigurationError):
            PortfolioManager(managers=[_FakePortfolioManager(name="A")], weights={"Unknown": 1.0})

    def test_rejects_negative_weight(self):
        with self.assertRaises(PortfolioManagerConfigurationError):
            PortfolioManager(managers=[_FakePortfolioManager(name="A")], weights={"A": -1.0})
    ...
    def test_accepts_zero_weight(self):
        manager = PortfolioManager(managers=[_FakePortfolioManager(name="A")], weights={"A": 0.0})
        self.assertEqual(manager.weights["A"], 0.0)

    def test_rejects_non_numeric_allow_threshold(self):
        with self.assertRaises(PortfolioManagerConfigurationError):
            PortfolioManager(allow_threshold="high")  # type: ignore[arg-type]

    def test_rejects_boolean_allow_threshold(self):
        with self.assertRaises(PortfolioManagerConfigurationError):
            PortfolioManager(allow_threshold=True)  # type: ignore[arg-type]

    def test_rejects_non_finite_allow_threshold(self):
        with self.assertRaises(PortfolioManagerConfigurationError):
            PortfolioManager(allow_threshold=float("nan"))

    def test_rejects_allow_threshold_out_of_range(self):
        with self.assertRaises(PortfolioManagerConfigurationError):
            PortfolioManager(allow_threshold=1.5)
        with self.assertRaises(PortfolioManagerConfigurationError):
            PortfolioManager(allow_threshold=-1.5)

    def test_accepts_boundary_allow_thresholds(self):
        PortfolioManager(allow_threshold=1.0)
        PortfolioManager(allow_threshold=-1.0)
...
class TestEvaluateValidation(unittest.TestCase):
    def test_rejects_non_portfolio_context(self):
        manager = PortfolioManager(managers=[_FakePortfolioManager(name="A")])
    ...
    def test_never_mutates_sub_manager_results(self):
        ...
        # frozen, but this also confirms the manager never rebuilds a
        # sub-result in place.
        mgr = _FakePortfolioManager(name="A", allowed=True, confidence=0.7)
        manager = PortfolioManager(managers=[mgr])
        ctx = make_context()
...
class TestEvaluateCompletenessAgreementConfidence(unittest.TestCase):
    ...
    def test_disagreement_lowers_confidence_via_agreement(self):
        agreeing = PortfolioManager(
            managers=[
                _FakePortfolioManager(name="A", allowed=True, confidence=0.9),
                _FakePortfolioManager(name="B", allowed=True, confidence=0.9),
            ]
        )
        disagreeing = PortfolioManager(
            managers=[
                _FakePortfolioManager(name="A", allowed=True, confidence=0.9),
...
class TestEvaluateOutputShape(unittest.TestCase):
    ...
    def test_result_is_a_valid_portfolio_result_with_four_fields(self):
        result = self.manager.evaluate(make_context())
        self.assertIsInstance(result, PortfolioResult)
        field_names = {f.name for f in result.__dataclass_fields__.values()}
        self.assertEqual(field_names, {"new_positions_allowed", "confidence", "summary", "metadata"})
    ...
    def test_metadata_contains_aggregation_details(self):
        ...
        self.assertEqual(details["allow_threshold"], self.manager.allow_threshold)

    def test_metadata_contains_top_level_required_facets(self):
        result = self.manager.evaluate(make_context())
        for key in ("aggregate_score", "completeness", "agreement"):
...
class TestRealBasicPortfolioManagerIntegration(unittest.TestCase):
    ...
    def test_disagreeing_basic_managers_reduce_agreement(self):
        ...
        # genuinely disagree.
        positions = [
            make_position(quantity=Decimal("1"), entry_price=Decimal("100"))
            for _ in range(4)
        ]
    ...
    def test_default_construction_uses_real_basic_portfolio_manager(self):
        ...
        result = PortfolioManager().evaluate(ctx)
        self.assertTrue(result.new_positions_allowed)
    ...
    def test_basic_manager_insufficient_data_treated_as_unavailable(self):
        ...
        # PortfolioManager must treat that as "unavailable" for that
        # sub-manager rather than failing the whole aggregation, as
        # long as at least one other sub-manager can still evaluate.
    ...
    def test_all_basic_managers_insufficient_data_raises(self):
        ...
        manager = PortfolioManager(managers=[BasicPortfolioManager()])
        with self.assertRaises(InsufficientPortfolioDataError):
            manager.evaluate(ctx)
