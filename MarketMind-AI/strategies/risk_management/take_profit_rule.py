"""
strategies/risk_management/take_profit_rule.py

Defines `TakeProfitRule`: a concrete `BaseRiskManager` implementation
(Risk Engine Part 3) that computes a target take-profit price for a
candidate `RiskContext`, direction-aware (above the reference price for
a `BUY` signal, below it for a `SELL` signal).

Independent of `PositionSizeRule` and `StopLossRule` -- it does not
import or depend on either. In particular, it does not consume
`StopLossRule`'s output; it estimates its own equivalent "risk leg"
distance internally (the same ATR-or-percentage approach `StopLossRule`
uses) purely to scale it by `risk_reward_ratio` into a target distance,
mirroring the independence already established between
...
Model:
    1. A reference (entry) price is resolved from
       ...
       (see below), this is *required* -- a take-profit is a price,
       and no price data on the context means no price can be
       produced.
    2. A base ("risk leg") distance, expressed as a fraction of the
       reference price, is estimated from ATR when available
       (`atr_value * atr_multiplier / reference_price`) or falls back
       to a configured `default_stop_distance_pct`, clamped to
       `[min_stop_distance_pct, max_stop_distance_pct]` -- identical in
       spirit to `StopLossRule`'s own distance estimate, computed
       independently here.
    3. The target distance is `base_distance_pct * risk_reward_ratio`.
    4. The take-profit price is `reference_price + target_distance` for
       a `BUY` signal, `reference_price - target_distance` for a `SELL`
       signal, and not applicable at all for `HOLD`.
...
`core.entities.position.Position.take_profit` -- this only recommends a
...
# Default constructor configuration -- see `TakeProfitRule.__init__`
...
#: Smallest positive price a computed take-profit is allowed to floor
#: to, so a wide distance on a low-priced instrument never produces a
#: non-positive price (relevant to the `SELL` direction, where the
#: target is subtracted from the reference price).
_MIN_PRICE_FLOOR = 1e-8


class TakeProfitRule(BaseRiskManager):
    """
    Computes a direction-aware take-profit price for a candidate
    signal, sized as a multiple of an internally-estimated risk
    distance.

    Attributes:
        atr_multiplier: Multiplier applied to the ATR reading (when
            available) to derive the base risk-leg distance.
        default_stop_distance_pct: Fallback base risk-leg distance,
            expressed as a fraction of the reference price, used when
            no ATR reading is available.
        min_stop_distance_pct: Minimum base risk-leg distance (as a
            fraction of price) the computed distance is clamped to,
            before scaling by `risk_reward_ratio`.
        max_stop_distance_pct: Maximum base risk-leg distance (as a
            fraction of price) the computed distance is clamped to,
            before scaling by `risk_reward_ratio`.
        risk_reward_ratio: Multiple applied to the base risk-leg
            distance to derive the take-profit target distance (e.g.
            `2.0` targets twice the base distance away).
    ...
    def __init__(
        ...
        *,
        ...
        if isinstance(risk_reward_ratio, bool) or not isinstance(risk_reward_ratio, (int, float)):
            raise RiskManagerConfigurationError(
                f"risk_reward_ratio must be numeric, got {type(risk_reward_ratio).__name__}"
            )
        if risk_reward_ratio <= 0.0:
            raise RiskManagerConfigurationError(
                f"risk_reward_ratio must be > 0.0, got {risk_reward_ratio}"
            )
    ...
    def evaluate(self, context: RiskContext) -> RiskResult:
        ...
        Raises:
            InsufficientRiskDataError: If `context.signal.confidence`
                ...
                take-profit is a price, and no price data means none
                can be computed.
        """
        ...
        if signal.direction == SignalDirection.HOLD:
            return self._build_result(
                ...
                summary="No take-profit computed: HOLD signal has no position to target.",
                metadata={
        ...
        if reference_price is None:
            raise InsufficientRiskDataError(
                ...
                "market_state.latest_candle.close) -- cannot compute a take-profit"
            )
        ...
        if atr_available:
            raw_base_distance_pct = (atr_value * self.atr_multiplier) / reference_price
            basis = "atr"
        else:
            raw_base_distance_pct = self.default_stop_distance_pct
            basis = "default_pct"

        base_distance_pct = clip(
            raw_base_distance_pct, self.min_stop_distance_pct, self.max_stop_distance_pct
        )
        base_distance_clamped = base_distance_pct != raw_base_distance_pct

        target_distance_pct = base_distance_pct * self.risk_reward_ratio
        target_distance = reference_price * target_distance_pct
        ...
        floored_to_min_price = take_profit_price < _MIN_PRICE_FLOOR
        if floored_to_min_price:
            take_profit_price = _MIN_PRICE_FLOOR

        max_target_distance_pct = self.max_stop_distance_pct * self.risk_reward_ratio
        risk_score = clip(
            target_distance_pct / max_target_distance_pct if max_target_distance_pct > 0 else 1.0
        )
        completeness = 1.0 if atr_available else 0.7
        confidence = clip(signal_confidence * completeness)

        summary = (
            f"Take-profit set at {take_profit_price:.8f} "
            f"({target_distance_pct * 100:.3f}% "
            f"{('above' if signal.direction == SignalDirection.BUY else 'below')} "
            f"reference price {reference_price:.8f}, "
            f"risk_reward_ratio={self.risk_reward_ratio:.2f}, basis={basis})"
        )
        ...
        metadata: dict[str, Any] = {
            ...
            "basis": basis,
            "raw_base_distance_pct": raw_base_distance_pct,
            "base_distance_pct": base_distance_pct,
            "base_distance_clamped": base_distance_clamped,
