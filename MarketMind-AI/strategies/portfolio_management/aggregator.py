"""
strategies/portfolio_management/aggregator.py

Defines `PortfolioAggregator`: Portfolio Management Part 4. Combines
multiple `PortfolioResult`s -- produced by one or more injected
`BasePortfolioManager` instances -- into a single, final
`PortfolioResult` for one `PortfolioContext`, mirroring
`strategies.aggregator.StrategyAggregator`'s shape one layer up.
...
`BasicPortfolioManager()` when none are supplied), runs each against
the given `PortfolioContext` in order, and merges their individual
...
`strategies/portfolio_management/basic_portfolio_manager.py`, or
`strategies/portfolio_management/portfolio_manager.py` (Portfolio
Management Parts 1-3) is modified or subclassed differently here.

Why this exists
----------------
`PortfolioManager` (Part 3) already combines multiple
`BasePortfolioManager` instances into one `PortfolioResult`.
`PortfolioAggregator` (Part 4) provides the same combining role built
explicitly to mirror `StrategyAggregator`'s naming, structure, and
documentation conventions one layer up, for callers/consumers that
standardize on the `*Aggregator` naming used by
`analysis.aggregator.AnalysisAggregator`, `signals.aggregator.
SignalAggregator`, and `strategies.aggregator.StrategyAggregator`.
`PortfolioAggregator` is itself a `BasePortfolioManager`, so it can be
injected anywhere a single portfolio manager is expected -- including
as a sub-manager of another `PortfolioAggregator` or `PortfolioManager`.

Weighted aggregation
---------------------
Each sub-manager is identified by its own `.name` (the same value
`BasePortfolioManager.__init__` already assigns/accepts) and may be
assigned a `weight` (default `1.0`, must be finite and `>= 0.0`) via
the `weights` constructor parameter, keyed by that name. A
sub-manager's own `confidence` further scales its contribution on top
of its fixed weight, the same "fixed weight x own confidence" shape
`StrategyAggregator`/`SignalAggregator` already use.

Since `PortfolioResult` deliberately carries no numeric score field
(only a boolean `new_positions_allowed`), each sub-manager's decision
is represented as a signed unit vote (`+1.0` allowed, `-1.0` blocked)
rather than a signed magnitude -- `confidence` is what scales how much
that unit vote counts, both in the aggregated score and in the
...
portfolio equity cannot be computed at all). `PortfolioAggregator`
catches that per-manager and treats it as "this manager produced no
decision" rather than failing the whole aggregation --
`metadata["managers_missing"]` records which managers were unavailable
and why. `PortfolioAggregator` itself only raises
...
`PortfolioResult.metadata` for full traceability, in particular:
    ...
    - `agreement`: weighted average of how much each contributing
      ...
      aggregate's final decision (`1.0` match, `0.0` mismatch -- a
      plain boolean agreement scale, since `new_positions_allowed` has
      no `HOLD`-style neutral third state the way `SignalDirection`
      does one layer up).
...
decision logic of its own beyond merging -- reuses Parts 1-3 exactly
as they exist, without modifying any of them.
"""
...
class PortfolioAggregator(BasePortfolioManager):
    """
    Combines the `PortfolioResult`s of one or more `BasePortfolioManager`
    instances into one final `PortfolioResult`, mirroring
    `strategies.aggregator.StrategyAggregator`'s shape one layer up.
    ...
    Parameters:
        ...
        weights: Optional per-manager weight overrides, keyed by each
            ...
            scale a manager's contribution to both the aggregated
            vote and the aggregated confidence. Must be finite and
            `>= 0.0`.
        ...
        name: Portfolio manager name, forwarded to
    ...
    def _aggregate_confidence(
        ...
        # `StrategyAggregator`'s completeness x agreement x confidence
        # shape: a single missing manager alone never zeroes out
        # confidence, but contributing managers that disagree with
        # each other legitimately produce a low one.
    ...
    def _validate_managers(
        ...
        if not items:
            raise PortfolioManagerConfigurationError(
                "PortfolioAggregator requires at least one manager; got an empty sequence."
            )
        ...
        for index, manager in enumerate(items):
            ...
            if manager.name in ordered:
                raise PortfolioManagerConfigurationError(
                    ...
                    "by PortfolioAggregator must have a unique name (pass a distinct "
...
# `analysis/technical/utils.py` provide for their own aggregators, and
# `strategies/portfolio_management/portfolio_manager.py` (Part 3)
# already defines equivalents -- duplicated locally here (rather than
# imported from that module) so this module has no dependency on
# Part 3, matching the sibling relationship `PortfolioManager` (Part 3)
# and `PortfolioAggregator` (Part 4) have to Parts 1-2.
